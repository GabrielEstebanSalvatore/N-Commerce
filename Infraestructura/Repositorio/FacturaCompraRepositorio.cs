...
{
    public class FacturaCompraRepositorio : Repositorio<Compra>, IFacturaCompraRepositorio
    {
        public FacturaCompraRepositorio(DataContext context)
           : base(context)
        {
        ...
        public override Compra Obtener(long entidadId, string propiedadNavegacion = "")
        {
            var resultado = propiedadNavegacion.Split(new[] { ',' },
                ...
                IQueryable<Compra>>(_context.Set<Comprobante>().OfType<Compra>(), (current, include)
        ...
        public override IEnumerable<Compra> Obtener(Expression<Func<Compra, bool>> filtro = null, string propiedadNavegacion = "")
        {
            ...
            var resultado = propiedadNavegacion.Split(new[] { ',' },
                StringSplitOptions.RemoveEmptyEntries).Aggregate<string,
                IQueryable<Compra>>(resultadoClient, (current, include) => current.Include(include));
