        ...
        public override long Insertar(ComprobanteDto comprobante)
        {
            using (var tran = new TransactionScope())
            {
                try
                {
                    ...
                    if (facturaDto.Estado == Estado.Pendiente && !facturaDto.VieneVentas && facturaDto.TipoComprobante != TipoComprobante.Presupuesto)
                    {
                    ...
                    else
                    {
                       numeroComprobante = _contadorServicio.ObtenerSiguienteNumeroComprobante(comprobante.TipoComprobante);
                        ...
                        foreach (var item in facturaDto.Items)
                        {
                            ...
                            if (facturaDto.TipoComprobante == TipoComprobante.Presupuesto && config.PresupuestoDescuentaStock)
                            {
                                var stockActual = _unidadDeTrabajo.StockRepositorio.Obtener(
                                   x => x.ArticuloId == item.ArticuloId
                            ...
                            if (config.FacturaDescuentaStock && facturaDto.TipoComprobante != TipoComprobante.Presupuesto && facturaDto.TipoComprobante != TipoComprobante.Remito)
                            {
                            ...
                            if (config.RemitoDescuentaStock ) 
                            {
                                var stockActual = _unidadDeTrabajo.StockRepositorio.Obtener(
                                    x => x.ArticuloId == item.ArticuloId
                                && x.DepositoId == config.DepositoVentaId).FirstOrDefault();
                    ...
                    if (facturaDto.Estado == Estado.Pagada && facturaDto.TipoComprobante != TipoComprobante.Remito)
                    {
                    ...
                    if (facturaDto.VieneVentas || facturaDto.vienePresupuesto)
                    {
                        if (facturaDto.TipoComprobante == TipoComprobante.Remito)
                        {
                    ...
                    {
                        _facturaNueva.TipoComprobante = comprobante.TipoComprobante;
